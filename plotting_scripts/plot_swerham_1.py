...
for j in range(0,2):
    ...
    if j == 0:
        ...
        axs[j].set_ylabel("Hamming error rate")
#        axs[j].set_ylim(0.0005,0.55)
    else:
        d_to_use = collapsed_swer_dict
        e_to_use = swer_stdev_dicts
        axs[j].set_ylabel("Switch error rate")
#        axs[j].set_ylim(0.0020,0.40)

    x = np.arange(len(covs))  # the label locations
    width = 0.35  # the width of the bars
    
    rects1 = axs[j].bar(x - width/4, d_to_use['flopp']['pacbio'], width/4, label='flopp-pacbio',color=flopp_colour,edgecolor='black', yerr = e_to_use['flopp']['pacbio'])
    rects2 = axs[j].bar(x - width/2, d_to_use['flopp']['nanopore'], width/4, label='flopp-nanopore',color=flopp_colour,hatch='//',edgecolor='black',yerr = e_to_use['flopp']['nanopore'])
    rects4 = axs[j].bar(x + width/2, d_to_use['hpop']['pacbio'], width/4, label='hpop-pacbio',color=hpop_colour,edgecolor='black',yerr = e_to_use['hpop']['pacbio'])
    rects3 = axs[j].bar(x + width/4, d_to_use['hpop']['nanopore'], width/4, label='hpop-nanopore',color=hpop_colour,hatch='//',edgecolor='black',yerr = e_to_use['hpop']['nanopore'])

    if 'nanopore' in d_to_use['whp']:
        rects5 = axs[j].bar(x + width, d_to_use['whp']['nanopore'], width/4, label='whp-nanopore',color=whp_colour,hatch='//',edgecolor='black',yerr = e_to_use['whp']['nanopore'])
        rects6 = axs[j].bar(x + width*5/4, d_to_use['whp']['pacbio'], width/4, label='whp-pacbio',color=whp_colour,edgecolor='black',yerr = e_to_use['whp']['pacbio'])

    rects = [rects1,rects2,rects3,rects4]
    axs[j].set_xticks(x)
    axs[j].set_xticklabels(covs)
    axs[j].set_xlabel("Coverage")
    ploidy = 3;
    if j == 1:
        title_str = "Switch error rate - "  + str(ploidy) + "x ploidy" 
        axs[j].set_title(title_str)
